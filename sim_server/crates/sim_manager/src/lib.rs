...
use simulator_binding::Simulator;
...
pub fn start<T: Send + Sync + 'static>(buffer: StateBuffer, _input_buffer: InputBuffer<T>) -> JoinHandle<()> {
    let folder_path = std::env::var("SIM_SCENE_FOLDER").unwrap_or_else(|_| "resources".to_string());
    thread::spawn(move || {
