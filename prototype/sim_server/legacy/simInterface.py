...
class PartState:
    ...
    @classmethod  # dump_frame()에서 만든 dict -> PartState
...
# PartState 직렬화 함수
_partstate_packer = struct.Struct('<7d')  # 7개의 double (x,y,z, e0,e1,e2,e3)

def serializePartState(state: PartState) -> bytes:
    """PartState를 bytes로 직렬화"""
    return _partstate_packer.pack(
...
class Simulation:
    ...
    models: Dict[int, Any]  # partId -> model object
    ...
    def step(
        ...
        if buffer is not None and hasattr(buffer, "read_outputs"):
            ...
            if isinstance(frame, dict):
                ...
                return ModelState(modelStates=model_states)
